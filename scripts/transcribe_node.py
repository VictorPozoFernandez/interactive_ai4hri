...
def callback(msg):
    ...
    if (len(utterance) > 12) and ("Amara.org" not in utterance) and ("for watching" not in utterance)  and ("Thank" not in utterance) and ("Gracias" not in utterance) and ("Merci" not in utterance) and ("Untertitel" not in utterance) and ("Sottotitoli" not in utterance) and ("ご視聴ありがとうございまし" not in utterance):
